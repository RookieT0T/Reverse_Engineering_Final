...
def generate(pattern, patternAddr, fileName):
    ...
    outReg20 = ", ".join(
        [f'[out{i}]"=v"(a)' for i in range(0, 40, 2)]
    )

    inReg20 = ", ".join(
        [f'[in{i * 2 + 1}]"v"((uint64_t *)&arr[{patternAddr[i]}])' for i in range(0, 20)]
    )

    # /////////////////////// #
    # the second 20 instructions
    ...
    outReg40 = ", ".join(
        [f'[out{i}]"=v"(b)' for i in range(40, 80, 2)]
    )

    inReg40 = ", ".join(
        [f'[in{i * 2 + 1}]"v"((uint64_t *)&arr[{patternAddr[i]}])' for i in range(20, 40)]
    )

    # /////////////////////// #
    # the third 20 instructions
    head60 = '\"s_waitcnt vmcnt(0) & lgkmcnt(0)\\n\\t\" \n\t\t\"buffer_wbinvl1\\n\\t\" \n\t\t\"flat_load_dwordx2 %[out80], %[in81] glc\\n\\t\"'
    instructions60 = "\n\n\t\t".join(
        [
            f'\"s_waitcnt vmcnt(0) & lgkmcnt(0)\\n\\t\" \n\t\t\"flat_load_dwordx2 %[out{i}], %[in{i + 1}] glc\\n\\t\"'
            for i in range(82, 120, 2)
        ]
    )
    final60 = head60 + "\n\n\t\t" + instructions60 + "\n\n\t\t" + tail
    
    outReg60 = ", ".join(
        [f'[out{i}]"=v"(c)' for i in range(80, 120, 2)]
    )

    inReg60 = ", ".join(
        [f'[in{i * 2 + 1}]"v"((uint64_t *)&arr[{patternAddr[i]}])' for i in range(40, 60)]
    )
...
for n in range(int(number)):
    ...
    for i in range(16, 60):
        ...
        if value == 0:                      # 0 -> access other cache lines that have not been accessed
            ...
            while(True):
                ...
                if offset[index] >= 16:         # set the upper limit of how many times each cache line can be accessed
                    print("skip")
                    continue
                # print(index)
                pattern.append(temp)
    ...
    generate(pattern, patternAddr, f'{directory}/trial_{n}.cpp')    # generate the testbench
